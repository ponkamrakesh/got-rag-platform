...
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col h-screen bg-iron-900">
      {/* Header */}
      <header className="flex items-center justify-between px-4 md:px-6 py-3 border-b border-iron-600 bg-iron-800/80 backdrop-blur shrink-0">
        <div className="flex items-center gap-3">
          <Flame className="text-fire-400 w-6 h-6" />
          <div>
            <h1 className="text-sm md:text-base font-bold tracking-widest text-zinc-100 font-serif">
              CITADEL RAG
            </h1>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          {hasSources && (
            <button
              ...
              className="sm:hidden flex items-center gap-1.5 px-3 py-2 rounded-md text-xs font-medium bg-iron-700 border border-iron-600 text-zinc-300"
            >
          ...
          <Link
            href="/dashboard"
            className="flex items-center gap-1.5 px-3 py-2 rounded-md text-xs font-medium bg-iron-700 border border-iron-600 text-zinc-300 hover:text-zinc-100 hover:border-zinc-500 transition"
          >
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <div className="flex flex-col flex-1 min-w-0">
          <div className="flex-1 overflow-y-auto scroll-thin p-4 md:p-6 space-y-5">
            {messages.map((m, i) => (
              ...
              >
                <div
                  className={`max-w-2xl rounded-lg px-4 py-3 md:px-5 md:py-4 leading-relaxed text-sm md:text-base whitespace-pre-wrap shadow-sm ${
                    m.role === 'user'
                      ? 'bg-fire-500/15 border border-fire-500/25 text-zinc-100'
                      : 'bg-iron-700 border border-iron-600 text-zinc-200'
                  }`}
                >
          ...
          <form
            ...
            className="p-3 md:p-4 border-t border-iron-600 bg-iron-800/50 shrink-0"
          >
            <div className="flex gap-2 md:gap-3 max-w-4xl mx-auto">
              <input
                ...
                className="flex-1 bg-iron-700 border border-iron-600 rounded-md px-3 md:px-4 py-2.5 md:py-3 text-sm placeholder-zinc-500 focus:outline-none focus:border-fire-500 transition"
              />
              <button
                type="submit"
                disabled={loading || streaming || !input.trim()}
                className="bg-fire-500 hover:bg-fire-400 disabled:opacity-40 text-white px-4 md:px-5 py-2.5 md:py-3 rounded-md transition flex items-center gap-2 font-medium text-sm shrink-0"
              >
        ...
        {hasSources && (
          <aside
            ...
            } xl:flex w-full xl:w-80 2xl:w-96 border-l border-iron-600 bg-iron-800/40 overflow-y-auto scroll-thin p-4 flex-col gap-3 absolute xl:relative z-20 h-full sm:max-w-sm sm:right-0 sm:ml-auto`}
          >
